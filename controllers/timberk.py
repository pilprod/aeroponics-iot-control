...
LOCKFILE = os.getenv("TIMBERK_LOCKFILE", "timberk.lock")
# Файл для хранения последнего отправленного кода
LAST_IR_CODE_FILE = os.getenv("TIMBERK_LAST_IR_CODE_FILE", "timberk-last-ir-code.txt")
# Логи
LOG_FILE = os.getenv("TIMBERK_LOG_FILE", "timberk.log")
...
TIMEOUT = 5  # Значение по умолчанию, можно изменить через аргумент командной строки
...
# MQTT топики
MQTT_TOPIC_POWER = os.getenv("MQTT_TOPIC_POWER", "portfolio/timberk/power")
MQTT_TOPIC_MODE = os.getenv("MQTT_TOPIC_MODE", "portfolio/timberk/mode")
MQTT_TOPIC_ION = os.getenv("MQTT_TOPIC_ION", "portfolio/timberk/ion")
MQTT_TOPIC_STEAM = os.getenv("MQTT_TOPIC_STEAM", "portfolio/timberk/steam")
MQTT_TOPIC_LIGHT = os.getenv("MQTT_TOPIC_LIGHT", "portfolio/timberk/light")
MQTT_TOPIC_NIGHT_MODE = os.getenv("MQTT_TOPIC_NIGHT_MODE", "portfolio/timberk/night_mode")
MQTT_TOPIC_IR_SEND = os.getenv("MQTT_TOPIC_IR_SEND", "portfolio/timberk/ir_send")
MQTT_TOPIC_LOG = os.getenv("MQTT_TOPIC_LOG", "portfolio/timberk/log")
MQTT_TOPIC_LAST_IR_CODE = os.getenv("MQTT_TOPIC_LAST_IR_CODE", "portfolio/timberk/last_ir_code")
MQTT_TOPIC_PHYSICAL_POWER = os.getenv("MQTT_TOPIC_PHYSICAL_POWER", "portfolio/timberk/physical_power")
...
def log_message(client, message):
    ...
    # Преобразуем лог в JSON-строку
    log_json = json.dumps(log_entry, ensure_ascii=False)

    # Публикация сообщения в MQTT в формате JSON
    if client is not None:
        client.publish(MQTT_TOPIC_LOG, log_json, qos=MQTT_QOS)

    # Запись логов в файл
    try:
        with open(LOG_FILE, 'a') as logfile:
            logfile.write(log_json + "\n")
...
def send_ir_signals(client, signals_list, signal_count, topic, state):
    log_message(client, f"Функция send_ir_signals вызвана с signal_count={signal_count}")
    if not signals_list:
    ...
    for i in range(signal_count):
        ...
        if ir_code:
            time.sleep(TIMEOUT)
            client.publish(MQTT_TOPIC_IR_SEND, ir_code, qos=2)
            log_message(client, f"Отправлен IR сигнал с кодом {ir_code}")
        else:
...
def update_steam_ion_state(client, target_steam, target_ion):
    ...
    log_message(client, f"Текущее состояние пара и ионизации: {current_state}")
    log_message(client, f"Целевое состояние пара и ионизации: {target_state}")
    log_message(client, f"STEAM_ION_STATES: {STEAM_ION_STATES}")
    ...
    log_message(client, f"Текущий индекс: {current_index}, Целевой индекс: {target_index}")

    if current_index is None or target_index is None:
        log_message(client, "Не удалось найти текущее или целевое состояние в STEAM_ION_STATES.")
        raise ValueError("Неправильное состояние пара или ионизации.")

    signals_needed = (target_index - current_index) % len(STEAM_ION_STATES)
    log_message(client, f"Необходимо отправить {signals_needed} сигналов для переключения ионизации и пара.")
    if signals_needed > 0:
        send_ir_signals(client, IR_SIGNALS_ION, signals_needed, MQTT_TOPIC_STEAM, target_steam)
    else:
        log_message(client, "Сигналы не требуются для переключения ионизации и пара.")

    # Обновляем состояния после отправки сигналов
    client.publish(MQTT_TOPIC_STEAM, target_steam, qos=MQTT_QOS, retain=True)
    client.publish(MQTT_TOPIC_ION, target_ion, qos=MQTT_QOS, retain=True)
    log_message(client, f"Состояния пара и ионизации обновлены: пар - {target_steam}, ионизация - {target_ion}")
...
def main():
    ...
    parser.add_argument("--timeout", type=int, default=5, help="Таймаут между сигналами (по умолчанию 20 секунд)")
    ...
    try:
        ...
        if not wait_for_states(timeout=5):
        ...
        if args.power_state == "on" and current_physical_power:
            ...
            if not current_power:
                ...
                time.sleep(3)  # Задержка после включения увлажнителя
            ...
            if current_night_mode != (args.night_mode_state == "on"):
                if args.night_mode_state == "off" and current_night_mode:
                    # Если хотим отключить ночной режим, и он сейчас включен, отправляем сигнал дважды
                    send_ir_signals(client, IR_SIGNALS_NIGHT_MODE, 2, MQTT_TOPIC_NIGHT_MODE, args.night_mode_state)
                else:
                    # В остальных случаях отправляем сигнал один раз
